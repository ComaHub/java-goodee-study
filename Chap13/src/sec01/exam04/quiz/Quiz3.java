...
// - 축구공: 3000원
	...
	public static void main(String[] args) {
		...
		Set<Equipment> addSet = new HashSet<Equipment>();
		addSet.addAll(storage1);
		addSet.addAll(storage2);
		
		System.out.println("*** 합집합 ***");
		Iterator<Equipment> iterator = addSet.iterator();
		while (iterator.hasNext()) {
			System.out.println("- " + iterator.next().toString());
		}
		
		Set<Equipment> retainSet = new HashSet<Equipment>();
		retainSet.addAll(storage1);
		retainSet.retainAll(storage2);
		
		System.out.println("*** 교집합 ***");
		iterator = retainSet.iterator();
		while (iterator.hasNext()) {
			System.out.println("- " + iterator.next().toString());
		}
		
		Set<Equipment> removeSet = new HashSet<Equipment>();
		removeSet.addAll(storage1);
		removeSet.removeAll(storage2);
