...
public class UUIDExample {
	public static void main(String[] args) {
		...
		// 실무 예시: 업로드 시 파일 이름 중복 방지
		// 사용자가 파일 업로드 시 동일한 이름의 파일이 존재하면 덮어쓰는 위험이 있으므로
		...
		if (!uploadDir.exists()) {
			...
			if (isDirCreated) System.out.println("생성 성공");
			else System.out.println("생성 실패");
		}
		
		String originName = "photo.png";
		...
		try {
			...
			if (created) System.out.println("저장 파일명: " + savedName);
			else System.out.println("파일이 이미 존재하거나 생성에 실패했습니다.");
